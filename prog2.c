...
unsigned short compute_checksum(message) 
struct msg message;
{
  int count = 20;
  register long sum = 0;
  char * addr = &message.data[0]; 
...
int corrupt(struct pkt pacote) {
  return pacote.checksum == compute_checksum(pacote.payload);
}
...
B_input(packet)
  struct pkt packet;
{
  if(corrupt(packet) || packet.seqnum != seq_B){
    return;
  }
